...
export default function Project() {
  ...
  const [showRequests, setShowRequests] = useState(false); // Requests toggle
      ...
      <Container maxWidth="lg">
        ...
        {value === 0 && (
          <>
            ...
            <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap", mt: 5 }}>
              <StatCard value={providerStats.total || 0} label="Total Projects" color="#00c853" progress={100} />
              <StatCard value={providerStats.active || 0} label="Active" color="#059669" progress={calculateProgress(providerStats.active, providerStats.total)} />
              <StatCard value={providerStats.pendingRequests || 0} label="Pending" color="#F59E0B" progress={calculateProgress(providerStats.pendingRequests, providerStats.total)} />
              <StatCard value={providerStats.completed || 0} label="Completed" color="#0284C7" progress={calculateProgress(providerStats.completed, providerStats.total)} />
              <StatCard value={providerStats.overdue || 0} label="Overdue" color="#DC2626" progress={calculateProgress(providerStats.overdue, providerStats.total)} />
            </Box>
              ...
              {filteredProviderProjects.length > 0 ? (
                <Grid container spacing={3}>
                  {filteredProviderProjects.map((project) => (
                    <Grid item xs={12} sm={6} lg={4} key={project.id}>
                      {showRequests ? (
                        <RequestProjectCard {...project} isProvider={true} />
                      ) : (
                ...
                <Box textAlign="center" py={8} bgcolor="#f9fafb" borderRadius={2}>
                  <Typography variant="h6" color="text.secondary">
                    {showRequests ? "No pending requests found" : "No projects found"}
                  </Typography>
        ...
        {value === 1 && (
          <>
            ...
            <Box sx={{ display: "flex", gap: 3, flexWrap: "wrap", mt: 5 }}>
              <StatCard value={clientStats.total || 0} label="Total Requests" color="#00c853" progress={100} />
              <StatCard value={clientStats.active || 0} label="Active" color="#059669" progress={calculateProgress(clientStats.active, clientStats.total)} />
              <StatCard value={clientStats.pendingRequests || 0} label="Pending" color="#F59E0B" progress={calculateProgress(clientStats.pendingRequests, clientStats.total)} />
              <StatCard value={clientStats.completed || 0} label="Completed" color="#0284C7" progress={calculateProgress(clientStats.completed, clientStats.total)} />
              <StatCard value={clientStats.overdue || 0} label="Overdue" color="#DC2626" progress={calculateProgress(clientStats.overdue, clientStats.total)} />
            </Box>
              ...
              {filteredClientProjects.length > 0 ? (
                <Grid container spacing={3}>
                  {filteredClientProjects.map((project) => (
                    <Grid item xs={12} sm={6} lg={4} key={project.id}>
                      {showRequests ? (
                        <RequestProjectCard {...project} isProvider={false} />
                      ) : (
                        <AllStatusProjectCard {...project} />
                      )}
                    </Grid>
