        ...
        <Container maxWidth="xl">
          <Toolbar sx={{ justifyContent: "space-between", width: "100%", bgcolor: "white", gap: "21px" }}>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
              <Box component="img" src={Logo} alt="logo" sx={{ width: 37, height: 37, cursor: "pointer" }} onClick={() => navigate("/")} />
              <Typography
                variant="h6"
                onClick={() => navigate("/")}
                sx={{
            ...
            <Box sx={{ display: "flex", alignItems: "center" }}>
              {/* Points */}
              {!admin && windowWidth >= 1029 && (
                <Box sx={{ display: "flex", alignItems: "center", gap: 1, bgcolor: "#3b82f638", borderRadius: 5, p: 1, mr: 2 }}>
                  <Box sx={{ width: 35, height: 35, backgroundColor: "#3B82F6", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(255, 255, 255, 1)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <Typography component="span" sx={{ fontWeight: "bold", color: "#3B82F6", fontSize: "20px" }}>
              ...
              {!admin && (
                <IconButton size="large" color="inherit" onClick={() => navigate("/chat")}>
                  <Badge badgeContent={unreadCount} color="error" max={99} sx={{ '& .MuiBadge-badge': { fontSize: '0.65rem', height: '18px', minWidth: '18px' } }}>
                    <MailIcon />
                  </Badge>
                </IconButton>
              )}
              ...
              {/* Account Menu */}
              <IconButton size="large" edge="end" aria-haspopup="true" onClick={handleProfileMenuOpen} color="inherit" sx={{ p: 0 }}>
                <Avatar
                ...
                {admin ? (
                  <>
                   {/* <MenuItem onClick={handleMenuClose}>
                    ...
                    {/* <Divider /> */}
                    <MenuItem onClick={() => { handleMenuClose(); logout(); navigate("/"); }} sx={{ color: "red" }}>
                      <ListItemIcon>
                  ...
                  <>
                    {/* Profile + Settings + Switch Admin + Sign Out for normal users */}
                    <Box sx={{ display: "flex", alignItems: "center", px: 2, py: 1, gap: 1 }}>
                      <Avatar alt={currentUser?.userName || "User"} src={getImageUrl(currentUser?.profilePicture, currentUser?.userName)} sx={{ width: 30, height: 30 }} />
                      <Box>
              ...
              {/* Mobile Menu Button */}
              {windowWidth < 1029 && (
                <IconButton size="large" aria-label="show more" onClick={toggleMobileMenu} color="inherit">
                  <MenuIcon />
                </IconButton>
              )}
            </Box>
      ...
      {mobileOpen && windowWidth < 1029 && (
        <Box sx={{ width: "100%", bgcolor: "white", borderBottom: 1, borderColor: "divider" }}>
          <Container maxWidth="xl" sx={{ py: 1 }}>
            <Button component={Link} to="/app/feed" fullWidth sx={{ justifyContent: "center" }}>Feed</Button>
            <Button component={Link} to="/app/browse" fullWidth sx={{ justifyContent: "center" }}>Browse</Button>
            {!admin && <Button component={Link} to="/app/project" fullWidth sx={{ justifyContent: "center" }}>Projects</Button>}
          </Container>
        </Box>
      )}
    </>
